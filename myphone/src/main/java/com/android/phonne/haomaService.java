...
import com.android.database.Simmsg;
import com.android.database.SimmsgService;
...
public class haomaService extends Service {
     ...
     * IMEI国际移动设备识别码（IMEI：International Mobile Equipment Identification Number）是区别移动设备的标志，储存在移动设备中，可用于监控被窃或无效的移动设备。
     * IMEI组成如下图所示，移动终端设备通过键入“*#06#”即可查得。其总长为15位，每位数字仅使用0～9的数字。其中TAC代表型号装配码，由欧洲型号标准中心分配；FAC代表装配厂家号码；SNR为产品序号，用于区别同一个TAC和FAC中的每台移动设备；SP是备用编码
     * IMSI国际移动用户识别码（IMSI：International Mobile Subscriber Identification Number）是区别移动用户的标志，储存在SIM卡中，可用于区别移动用户的有效信息
     * IMSI组成如下图所示，其总长度不超过15位，同样使用0～9的数字。 其中MCC是移动用户所属国家代号，占3位数字，中国的MCC规定为460；MNC是移动网号码，最多由两位数字组成，用于识别移动用户所归属的移动通信网；MSIN是移动用户识别码，用以识别某一移动通信网中的移动用户。
    ...
    public void getPhoneNumber(Context context) {
        ...
        String imei = mTelephonyMgr.getSimSerialNumber();  //IMEI 国际移动设备识别码,SIM卡的序列号
        String imsi = mTelephonyMgr.getSubscriberId();     //IMSI 国际移动用户识别码 ,SIM卡唯一标识
        int SimState = mTelephonyMgr.getSimState();        //SIM卡状态
        simInfo = "tel:" + tel + ",imei:" + imei + ",imsi:" + imsi + ",deviceid:" + deviceid + ",SimState:" + SimState;
